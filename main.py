...
def traverse_tanks(tanks: dict, current_tank_id: str, goal_tank_id: str, 
                    current_path: tuple[str], current_cost: Cost, indent=0) -> Cost:
    ...
    field = indent * 4 * " "
    print(f"{field}current tank: {tanks[current_tank_id]["name"]}")

    min_cost, min_path = None, None
    for next_tank_id, research_cost in next_tanks.items():
        print(f"{field}next tank: {tanks[next_tank_id]["name"]}")
        print(f"{field}{current_cost}")
        print(f"{field}{research_cost}")
        print(f"{field}{current_cost + research_cost}")
        next_cost, next_path = traverse_tanks(tanks, next_tank_id, goal_tank_id,
                                              current_path + (next_tank_id,),
                                              current_cost + research_cost, indent+1)
        if next_cost is None:
            print(f"{field}wrong path")
            continue
        if min_cost is None or next_cost < min_cost:
            min_cost = next_cost
            min_path = next_path
            print(f"{field}good path")
